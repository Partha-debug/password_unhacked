...
def pswd_check_main():
    ...
    if (strength_profiles['lnth'] >= 8) & (strength_profiles['smal'] >=2) & (strength_profiles['cptl'] >=2) & (strength_profiles['num'] >=2) & (strength_profiles['spcl_chr'] >=2):
        print(f"Your password is strong it is of {strength_profiles['lnth']} characters contains {strength_profiles['smal']} small {strength_profiles['cptl']} capital {strength_profiles['num']} numeric and {strength_profiles['spcl_chr']} special characters.") 
        ...
        print(f"[-] Your password is of medium strength it is of {strength_profiles['lnth']} characters contains {strength_profiles['smal']} small {strength_profiles['cptl']} capital {strength_profiles['num']} numeric and {strength_profiles['spcl_chr']} special characters.\n\n[+] A Strong password must contain atleast 2 capital, 2 small,2 numeric and 2 special characters...") 
    else:
        print(f"[-] Your password is weak it is of {strength_profiles['lnth']} characters contains {strength_profiles['smal']} small {strength_profiles['cptl']} capital {strength_profiles['num']} numeric and {strength_profiles['spcl_chr']} special characters.\n\n[+]A Strong password must contain atleast 2 capital, 2 small,2 numeric and 2 special characters...")
...
def pwned_check_main():
    ...
    try:
        ...
        if pwned_info:
            print(f"Your password was found in {pwned_info} hacked password data bases, change it as soon as possible...")
        else:
            print("You got a secure password, There is no instance of this password in the hacked password databases...")
...
def pswd_gen_main():
    ...
    print(
        f"Generating a strong password with {provided_length} characters... ")
    ...
    print(
        f"Your generated {used_site} password is '{generated_pswd}', It will be saved in the passwords.txt file")
